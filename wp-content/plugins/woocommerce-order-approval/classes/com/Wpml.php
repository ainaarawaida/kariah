...
class Wpml
{
	...
	{
		if(!isset($_POST['wcoa_wpml_language']) || !$this->wpml_is_active())
			return;
		
		global $sitepress;
		load_plugin_textdomain('woocommerce-product-fields-checkout', false, WCOA_PLUGIN_LANG_PATH );
		$sitepress->switch_lang($_POST['wcoa_wpml_language'], true);
	}
	...
	public function get_all_translation_ids($post_id, $post_type = "product")
	{
		...
		//if($post_type == "product")
		{
			...
			//wcoa_var_dump($translations_result);
		}
	...
	/* function unregister_fields($fields)
	{
		if (function_exists ( 'icl_unregister_string' ) && class_exists('SitePress'))
			foreach((array)$fields as $old_field)
				{
					icl_unregister_string ( 'woocommerce-product-fields-checkout', 'wcoa_'.$old_field->cid );
					if(isset($old_field->field_options->options))
						foreach($old_field->field_options->options as $index => $extra_option)
						{
							if(isset($extra_option->label))
								icl_unregister_string ( 'woocommerce-product-fields-checkout','wcoa_'.$old_field->cid."_sublabel_".$index);
						}
					if(isset($old_field->field_options->description))
							icl_unregister_string ( 'woocommerce-product-fields-checkout', 'wcoa_'.$old_field->cid.'_description');
				}
	}*/
	function translate_single_string($id, $text, $prefix = "wcoa_label_"  )
	{
		if(!class_exists('SitePress'))
			return $text;
		
		$result = apply_filters( 'wpml_translate_single_string', $text, 'woocommerce-product-fields-checkout', 'wcoa_'.$prefix.$id, ICL_LANGUAGE_CODE  );
			
		return $result != "" ? $result : $text;
	}
	function register_single_string($id, $text, $prefix = "wcoa_label_" )
	{
		do_action( 'wpml_register_single_string', 'woocommerce-product-fields-checkout', 'wcoa_'.$prefix.$id, $text);
	}
	...
	public function get_langauges_list()
	{
		 ...
		 */
		 ...
		 $tmp_result = !$this->wpml_is_active() || empty($langs) ? array(0 => array("id"=> 0, "code" => 0, "language_code"=> $this->get_current_language(), "default_locale" => $this->get_default_locale(), "country_flag_url" => "none")) : $langs;
		 $result = array();
		 foreach($tmp_result as $lang_data)
			$result[$lang_data['language_code']] = $lang_data;
