	...
	public function processing() {
		...
		if(!empty($wcfm_subscriptions_array)) {
			...
			foreach($wcfm_subscriptions_array as $wcfm_subscriptions_single) {
				$the_subscription = wcs_get_subscription( $wcfm_subscriptions_single->ID );
				$the_order   = wc_get_order( $wcfm_subscriptions_single->post_parent );
				
				// Status
				$wcfm_subscriptions_json_arr[$index][] =  apply_filters( 'wcfm_subscription_status_label', '<span class="subscription-status tips wcicon-status-' . sanitize_title( $the_subscription->get_status( ) ) . ' text_tip" data-tip="' . $wc_get_subscription_status_name[$the_subscription->get_status()] . '"></span>', $wcfm_subscriptions_single->ID, $the_subscription );
