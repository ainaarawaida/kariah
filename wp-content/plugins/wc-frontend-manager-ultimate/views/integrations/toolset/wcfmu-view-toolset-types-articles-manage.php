 ...
 * WCFM plugin views
 *
 * Plugin Toolset Types Articles Manage Views
 *
 * @author 		WC Lovers
...
if( !apply_filters( 'wcfm_is_allow_toolset_types', true ) || !apply_filters( 'wcfm_is_allow_toolset_types_article', true ) ) {
	return;
}
...
if( !$article_id ) {
	//$article_post = get_default_post_to_edit( 'post', false );
}

$article_post = get_post();
...
if( !empty( $field_groups )) {
    ...
    if( !$wcfm_is_allowed_toolset_field_group ) continue;
		...
		if ( !empty( $field_group['fields'] ) ) { 
			?>
			<div class="page_collapsible wcfm_toolset_articles_manage_collapsible wcfm_toolset_articles_manage_<?php echo $field_group['id']; ?>_collapsible simple variable external grouped booking" id="wcfm_articles_manage_form_<?php echo $field_group['id']; ?>_head"><label class="wcfmfa fa-certificate"></label><?php echo $field_group['name']; ?><span></span></div>
			<div class="wcfm-container simple variable external grouped booking wcfm_toolset_articles_manage_container wcfm_toolset_articles_manage_<?php echo $field_group['id']; ?>_container">
				<div id="wcfm_articles_manage_form_<?php echo $field_group['id']; ?>_expander" class="wcfm-content">
				  ...
				  if ( !empty( $field_group['fields'] ) ) {
				  	foreach( $field_group['fields'] as $field_group_field ) {
				  		...
				  		if( $article_id ) $field_value = get_post_meta( $article_id, $field_group_field['meta_key'], true );
				  		...
				  		if ( wpcf_admin_is_repetitive( $field_group_field ) ) {
				  			$field_value = array();
				  			$field_value_repetatives = (array) get_post_meta( $article_id, $field_group_field['meta_key'] );
									...
									if( WCFMu_Dependencies::wcfm_toolset_address_map_active_check() ) {
										if( apply_filters( 'wcfm_is_allow_toolset_address_map', true ) ) {
											...
											<div class="toolset-google-map-container js-toolset-google-map-container">
											 ...
											 <style>
												 .wcfm_toolset_articles_manage_<?php echo $field_group['slug']; ?>_container .toolset-google-map-inputs-container {
															width: 52% !important;
															float: left;
													}
												 .wcfm_toolset_articles_manage_<?php echo $field_group['slug']; ?>_container .toolset-google-map-preview {
														width: 45% !important;
														height: 200px;
														float: right;
													}
											...
											wp_localize_script(
													...
													array(
																...
																'wcfmtoolsetmapsblockhead' => 'wcfm_articles_manage_form_' . $field_group['slug'] . '_head'
																)
													);
