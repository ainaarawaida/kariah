 ...
 * WCFM plugin views
 *
 * Plugin Advanced Custom Fields(ACF) Pro Articles Manage Views
 *
 * @author 		WC Lovers
...
wp_localize_script( 'wcfmu_acf_pro_articles_manage_js', 'wcfm_cat_based_acf_pro_fields', $cat_group_id_map );
...
if( !empty( $field_groups )) {
	foreach( $field_groups as $field_group_index => $field_group ) {
		...
		$cat_group_class = '';
		if( in_array( $field_group['ID'], $cat_group_ids ) ) $cat_group_class = 'wcfm_cat_based_acf_pro_article_manager_fields';
							  ...
							  break;
								...
								case 'user':
									...
									if( isset( $field_group_field['multiple'] ) && ( $field_group_field['multiple'] == 1 ) ) {
										...
										$WCFM->wcfm_fields->wcfm_generate_form_field(  array( $field_group_field['name'] => array( 'label' => $field_group_field['label'], 'custom_attributes' => $custom_attributes, 'hints' => $field_group_field['instructions'] , 'name' => 'acf[' . $field_group_field['name'] . ']', 'type' => 'select', 'class' => 'wcfm-select wcfm_ele simple variable external grouped booking', 'label_class' => 'wcfm_title', 'options' => $user_arr, 'value' => $field_value ) ) );
									}
								break;
