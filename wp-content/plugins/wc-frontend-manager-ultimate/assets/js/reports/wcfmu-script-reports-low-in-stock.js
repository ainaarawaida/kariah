jQuery(document).ready(function($) {
	$wcfm_products_table = $('#wcfm-reports-low-in-stock').DataTable( {
		"processing": true,
		"serverSide": true,
		...
		'ajax': {
			...
			"data"   : function( d ) {
				...
				d.controller = 'wcfm-reports-low-in-stock'
			},
			"complete" : function () {
				initiateTip();
	...
	function deleteWCFMProduct(item) {
		jQuery('#wcfm-reports-low-in-stock_wrapper').block({
			message: null,
			overlayCSS: {
				background: '#fff',
				opacity: 0.6
		...
		jQuery.ajax({
			...
			success:	function(response) {
				...
				jQuery('#wcfm-reports-low-in-stock_wrapper').unblock();
			}
		});
	}
