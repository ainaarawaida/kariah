<?php
/**
 * WCFM Create new bookings page.
 */
class WCFM_Create_Bookings_Manual {
	...
	public function output() {
		...
		try {
			...
			if ( ! empty( $_POST['create_booking'] ) ) {
				...
				$new_booking = apply_filters( 'wcfm_manual_bookings_props', $new_booking );
				$new_booking->save();
				...
				?>
				<script>
				  window.location = '<?php echo apply_filters( 'wcfm_manual_booking_redirect', get_wcfm_view_booking_url( $new_booking->get_id() ), $new_booking->get_id() ); ?>';
				</script>
				<?php
				exit;
	...
	public function create_order( $total, $customer_id ) {
		if ( version_compare( WC_VERSION, '3.0', '<' ) ) {
			...
			update_post_meta( $order_id, '_created_via', 'bookings' );
		} else {
			$order = new WC_Order();
