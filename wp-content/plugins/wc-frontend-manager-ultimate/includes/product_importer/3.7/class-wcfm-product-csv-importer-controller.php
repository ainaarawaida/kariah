...
class WCFM_Product_CSV_Importer_Controller {
	...
	protected function output_errors() {
		...
		foreach ( $this->errors as $error ) {
			echo '<div class="wcfm-message wcfm-error" style="display: block;"><p><span class="wcicon-status-cancelled"></span>' . esc_html( $error['message'] ) . '</p></div>';
	...
	public function import() {
		...
		if ( ! empty( $_POST['map_from'] ) && ! empty( $_POST['map_to'] ) ) {
			...
			?>
			<script>
				window.location = '<?php echo esc_url_raw( $this->get_next_step_link( 'upload' ) ); ?>';
			</script>
			<?php
