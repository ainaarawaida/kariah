...
class WCFM_Bookings_Calendar {
	...
	public function list_bookings( $day, $month, $year ) {
		...
		foreach ( $this->events as $event ) {
			...
			if ( 'WC_Booking' === get_class( $event ) ) {
				...
				$detail_href = get_wcfm_view_booking_url( $booking->get_id() );
		...
		if ( 0 < $remaining_bookings_count ) {
			...
			$day_view_url = get_wcfm_bookings_calendar_url( 'day', date( 'Y-m-d', $date_start ) );
	...
	protected function get_booking_data( $event, $check_date = null ) {
		...
		if ( 'WC_Booking' === get_class( $event ) ) {
			...
			$event_title      = $product ? $booking->get_product()->get_name() : '';
			$event_url        = get_wcfm_view_booking_url( $booking->get_id() );
