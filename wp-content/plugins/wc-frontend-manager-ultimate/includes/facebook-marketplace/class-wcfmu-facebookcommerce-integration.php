...
if ( ! class_exists( '\\SkyVerge\\WooCommerce\\PluginFramework\\v5_10_0\\SV_WC_Plugin_Exception' ) ) return;
if ( ! class_exists( '\\SkyVerge\\WooCommerce\\PluginFramework\\v5_10_0\\SV_WC_Helper' ) ) return;

// require_once 'facebook-config-warmer.php';
// require_once facebook_for_woocommerce()->get_plugin_path() . '/includes/fbproduct.php';
// require_once 'facebook-commerce-pixel-event.php';

class WCFMu_Facebookcommerce_Integration extends WC_Integration {
	...
	/** @var string the WordPress user meta key where the product catalog ID is stored */
	const OPTION_PRODUCT_CATALOG_ID = 'wcfm_facebook_product_catalog_id';

	/** @var string the WordPress option name where the external merchant settings ID is stored */
	const OPTION_EXTERNAL_MERCHANT_SETTINGS_ID = 'wcfm_facebook_external_merchant_settings_id';
	...
	const SETTING_FACEBOOK_PAGE_ID = 'wcfm_facebook_page_id';

	/** @var string the facebook pixel ID setting ID */
	const SETTING_FACEBOOK_PIXEL_ID = 'wcfm_facebook_pixel_id';
	...
	const SETTING_EXCLUDED_PRODUCT_CATEGORY_IDS = 'wcfm_facebook_excluded_product_category_ids';

	/** @var string the excluded product tag IDs setting ID */
	const SETTING_EXCLUDED_PRODUCT_TAG_IDS = 'wcfm_facebook_excluded_product_tag_ids';
	...
	const SETTING_ENABLE_MESSENGER = 'wcfm_facebook_enable_messenger';

	/** @var string the messenger locale setting ID */
	const SETTING_MESSENGER_LOCALE = 'wcfm_facebook_messenger_locale';
	...
	public function __construct() {
		...
		if ( ! class_exists( 'WC_Facebookcommerce_Utils' ) ) {
			include_once facebook_for_woocommerce()->get_plugin_path() . '/includes/fbutils.php';
		}

		WC_Facebookcommerce_Utils::$ems = $this->get_external_merchant_settings_id();

		if ( ! class_exists( 'WC_Facebookcommerce_Graph_API' ) ) {
			include_once facebook_for_woocommerce()->get_plugin_path() . '/includes/fbgraph.php';
		}

		$this->fbgraph = new WC_Facebookcommerce_Graph_API( $WCFMu->wcfmu_facebook_marketplace->get_connection_handler( $this->vendor_id )->get_access_token() );
		...
		if ( is_admin() ) {
			...
			&& $this->get_pixel_install_time() ) {
				...
				if ( $should_query_tip || $last_tip_info ) {
					if ( ! class_exists( 'WC_Facebookcommerce_Info_Banner' ) ) {
						include_once facebook_for_woocommerce()->get_plugin_path() . '/includes/fbinfobanner.php';
					}
			...
			if ( ! class_exists( 'WC_Facebook_Integration_Test' ) ) {
				include_once facebook_for_woocommerce()->get_plugin_path() . '/includes/test/facebook-integration-test.php';
			}
	...
	public function load_background_sync_process() {
		...
		include_once facebook_for_woocommerce()->get_plugin_path() . '/includes/fbbackground.php';
	...
	public function ajax_fb_background_check_queue() {
		...
		if ( $WCFMu->wcfmu_facebook_marketplace->get_connection_handler( $this->vendor_id )->get_access_token() ) {
			...
			if ( ! empty( $this->get_upload_id() ) ) {
				...
				if ( ! isset( $this->fbproductfeed ) ) {
					...
					if ( ! class_exists( 'WC_Facebook_Product_Feed' ) ) {
						include_once facebook_for_woocommerce()->get_plugin_path() . '/includes/fbproductfeed.php';
					}
	...
	public function get_page_access_token() {
		 ...
		 * @param \WCFMu_Facebookcommerce_Integration $integration the integration instance
		 */
	...
	public function get_product_catalog_id() {
		...
		$value = get_user_meta( $this->vendor_id, self::OPTION_PRODUCT_CATALOG_ID, true );
		 ...
		 * @param \WCFMu_Facebookcommerce_Integration $integration the integration instance
		 */
		return (string) apply_filters( 'wcfm_facebook_product_catalog_id', $this->product_catalog_id, $this );
	}
	...
	public function get_external_merchant_settings_id() {
		...
		$value = get_user_meta( $this->vendor_id, self::OPTION_EXTERNAL_MERCHANT_SETTINGS_ID, true );
		 ...
		 * @param \WCFMu_Facebookcommerce_Integration $integration the integration instance
		 */
		return (string) apply_filters( 'wcfm_facebook_external_merchant_settings_id', $this->external_merchant_settings_id, $this );
	}
	...
	public function get_facebook_page_id() {
		...
		$facebook_page_id = get_user_meta( $this->vendor_id, self::SETTING_FACEBOOK_PAGE_ID, true );
		 ...
		 * @param \WCFMu_Facebookcommerce_Integration $integration the integration instance
		 */
		return (string) apply_filters( 'wcfm_facebook_page_id', $facebook_page_id, $this );
	}
	...
	public function get_facebook_pixel_id() {
		...
		$facebook_pixel_id = get_user_meta( $this->vendor_id, self::SETTING_FACEBOOK_PIXEL_ID, true );
		 ...
		 * @param \WCFMu_Facebookcommerce_Integration $integration the integration instance
		 */
		return (string) apply_filters( 'wcfm_facebook_pixel_id', $facebook_pixel_id, $this );
	}
	...
	public function get_excluded_product_category_ids() {
		...
		$vendor_data = get_user_meta( $this->vendor_id, 'wcfm_facebook_marketplace_settings', true );

		$product_sync = isset( $vendor_data['product_sync'] ) ? $vendor_data['product_sync'] : array();

		$excluded_product_category_ids = isset( $product_sync['excluded_product_category_ids'] ) ? $product_sync['excluded_product_category_ids'] : array();
		 ...
		 * @param \WCFMu_Facebookcommerce_Integration $integration the integration instance
		 */
		return (array) apply_filters( 'wcfm_facebook_excluded_product_category_ids', $excluded_product_category_ids, $this );
	}
	...
	public function get_excluded_product_tag_ids() {
		...
		$excluded_product_tag_ids = isset( $product_sync['excluded_product_tag_ids'] ) ? $product_sync['excluded_product_tag_ids'] : array();
		 ...
		 * @param \WCFMu_Facebookcommerce_Integration $integration the integration instance
		 */
		return (array) apply_filters( 'wcfm_facebook_excluded_product_tag_ids', $excluded_product_tag_ids, $this );
	}
	...
	public function get_messenger_locale() {
		...
		$messenger_locale = get_user_meta( $this->vendor_id, self::SETTING_MESSENGER_LOCALE, true ) ?: 'en_US';
		 ...
		 * @param \WCFMu_Facebookcommerce_Integration $integration the integration instance
		 */
		return (string) apply_filters( 'wcfm_facebook_messenger_locale', $messenger_locale, $this );
	}
	 ...
	 * @since 2.0.0
	 *
	 * @param string $value the Produxt Catalog ID
	 */
	public function update_product_catalog_id( $value ) {

		$this->product_catalog_id = $this->sanitize_facebook_credential( $value );

		update_user_meta( $this->vendor_id, self::OPTION_PRODUCT_CATALOG_ID, $this->product_catalog_id );
	}
	 ...
	 * @since 2.0.0
	 *
	 * @param string $value the Facebook Page ID
	 */
	public function update_facebook_page_id( $value ) {

		update_user_meta( $this->vendor_id, self::SETTING_FACEBOOK_PAGE_ID, $value );
	}
	 ...
	 * @since 2.0.0
	 *
	 * @param string $value the Facebook Pixel ID
	 */
	public function update_facebook_pixel_id( $value ) {

		update_user_meta( $this->vendor_id, self::SETTING_FACEBOOK_PIXEL_ID, $value );
	}
	...
	public function update_external_merchant_settings_id( $value ) {
		...
		update_user_meta( $this->vendor_id, self::OPTION_EXTERNAL_MERCHANT_SETTINGS_ID, $this->external_merchant_settings_id );
	}
	...
	public function is_configured() {
		...
		return $this->get_facebook_page_id() && $WCFMu->wcfmu_facebook_marketplace->get_connection_handler( $this->vendor_id )->is_connected();
	}
	...
	public function is_messenger_enabled() {
		...
		$messenger_enabled = get_user_meta( $this->vendor_id, self::SETTING_MESSENGER_LOCALE, true ) ?: 'no';
		 ...
		 * @param \WCFMu_Facebookcommerce_Integration $integration the integration instance
		 */
		return (bool) apply_filters( 'wcfm_facebook_is_messenger_enabled', 'yes' === $messenger_enabled, $this );
	}
	 ...
	 * @internal
	 *
	 * @see \WCFMu_Facebookcommerce_Integration::schedule_resync()
	 *
	 * @since 1.10.0
	 */
