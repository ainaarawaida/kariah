(function ($, window, document, undefined) {
    ...
    var FBC_console = "fbc_console",

    // The name of using in .data()
        data_plugin = "plugin_" + FBC_console,
    ...
    // Default options
        ...
        premium = {
            ...
            premium_console   : function () {
                ...
                for (i = 0; i < sound_set.length; ++i) {
                    ...
                    var source = document.createElement('source'),
                        name = sound_set[i],
                        filename = fbc.plugin_url + '/sounds/' + name;
                            ...
                            if (msgs) {
                                ...
                                $.each(msgs, function (msg_id, msg) {
                                    ...
                                    if (total_msgs === i) {
                                        ...
                                        self.post('fbc_ajax_callback', 'save_chat', user_data, function (r) {
            ...
            set_avatar_premium: function (user_type, user_data) {
                ...
                if (user_type != 'admin')
                    return 'https://www.gravatar.com/avatar/' + user_data.gravatar + '.jpg?s=60&d=' + fbc.default_user_avatar;

                switch (user_data.avatar_type) {

                    case 'gravatar':
                        return 'https://www.gravatar.com/avatar/' + user_data.gravatar + '.jpg?s=60&d=' + fbc.default_admin_avatar;
                        break;
                    ...
                    default:
                        ...
                        if (fbc.company_avatar != '') {

                            return fbc.company_avatar;
    ...
    function Plugin() {
        ...
        this.opts = $.extend(defaults, fbc.defaults);
    ...
    Plugin.prototype = {
        ...
        init              : function () {
            ...
            this.data = {
                ...
                assets_url    : fbc.plugin_url,
                user          : {}, 	        // User data
                online_ops    : {} 	        // Online operators list
            };

            this.strings = fbc.strings;
            ...
            $('#FBC_connect').click(function (e) {

                e.preventDefault();

                $('#FBC_notify').show().html(self.strings.msg.connecting + '...');
            ...
            this.post('fbc_ajax_callback', 'get_token', {}, function (r) {
                if (!r.error) {
        ...
        after_load        : function () {
            ...
            $(document).on('keydown', '#FBC_cnv_reply', function () {
                $(this).trigger('autosize.resize');
                $(window).trigger('resize');
            });

            $(document).on('focus', '#FBC_cnv_reply', function () {
                $(this).autosize({
                    append: ''
                });
            });
             ...
             */
            $(document).on('click', '#FBC_users li.free, #FBC_users li.busy', function () {
                ...
                self.get_user_data($(this).data('id'), function (user) {
                    ...
                    if (self.data.active_user_id)
                        $('#FBC_chat_user_' + self.data.active_user_id).removeClass('chat-active');

                    obj_user.addClass('chat-active').removeClass('new-msg').data('count', 0).find('.chat-count').empty();

                    $('#FBC_popup_cnv').removeClass('chat-welcome');


                    $('#FBC_cnv_reply').val('').focus().removeAttr('disabled');
                    ...
                    $('#FBC_end_chat').attr('data-cnv-id', obj_user.data('cnv-id'));
                    $('#FBC_save').attr('data-cnv-id', obj_user.data('cnv-id'));
                    $('#FBC_active_cnv').val(obj_user.data('cnv-id'));

                    self.objs.cnv = $('#FBC_cnv');
                    self.objs.cnv.html('');
             ...
             * End chat
             */
            $(document).on('click', '#FBC_save, #FBC_end_chat', function (e) {

                var btn = $(this),
                    ntf = $('#FBC_save_ntf'),
                    delete_from_app = ( $(this).attr('id') === 'FBC_end_chat' ),
                    now = new Date(),
                ...
                if (fbc.is_premium) {
                    ...
                    self.trigger_premium('end_chat_console', $('#FBC_active_cnv').val(), delete_from_app, end_chat, btn, ntf);

                } else {

                    self.clear_user_data($('#FBC_active_cnv').val(), function () {
             ...
             */
            $('#FBC_popup_cnv').mouseover(function () {

                $('#FBC_chat_user_' + self.data.active_user_id).removeClass('new-msg')
                    .data('count', 0)
            ...
            setInterval(function () {
                ...
                if (new_wait > 0) {
                    ...
                    $('#FBC_queue').html(self.strings.msg.waiting_users.replace(/%d/i, new_wait)).show();
        ...
        login             : function (new_user, callback) {
            ...
            this.data.auth = this.data.ref.authWithCustomToken(this.data.auth_token, function (error) {
                ...
                if (error) {
                    ...
                    $('#FBC_connect').removeClass('button-disabled');

                    $('#FBC_notify').hide().html(error.message).fadeIn(200);

                    self.display_ntf(self.strings.msg.conn_err, 'error');

                } else {

                    $('#FBC_notify').html(self.strings.msg.you_offline);

                    $('#FBC_connect').html(self.strings.msg.connect)
                        .data('logged', 0)
        ...
        logged_in         : function (user) {
            ...
            $('#FBC_notify').hide().empty();

            $('#FBC_connect').html('<i class="fa fa-check-circle" style="color:#acc327;"></i> ' + self.strings.msg.online_btn)
                .data('logged', 1)
                .data('status', 'online')
                .removeClass('button-disabled');
        ...
        offline           : function () {
            ...
            $('#FBC_connect').html('<i class="fa fa-check-circle" style="color:#e54045;"></i> ' + self.strings.msg.offline_btn)
                .data('logged', 0)
                .data('status', 'offline')
                .removeClass('button-disabled');
        ...
        get_user          : function (user_id, user_data, callback) {
            ...
            if (user_data.user_id) {
                ...
                this.data.ref_user.child('vendor_id').set(fbc.active_vendor.vendor_id);
                this.data.ref_user.child('vendor_name').set(fbc.active_vendor.vendor_name);
                ...
                // Prepare user data
                    data = {
                        ...
                        vendor_id      : fbc.active_vendor.vendor_id,
                        vendor_name    : fbc.active_vendor.vendor_name
                    };
        ...
        listen_users      : function () {
            ...
            // Add ul list
            $('#FBC_users').append('<ul></ul>');

            // Select list
            this.data.user_list = $('#FBC_users > ul');
            ...
            if (user) {
                ...
                if (user.conversation_id) {
                    ...
                    if (this.data.active_user_id === user.user_id)
                        $('#FBC_active_page').attr('href', user.current_page).find('span').html(user.current_page);
        ...
        add_user_item     : function (user) {
            ...
            var username = '',
                is_busy = false,
                elem_id = '#FBC_chat_user_' + user.user_id;
            ...
            if (fbc.wcfm_wpv_active && fbc.active_vendor.vendor_id == '0') {
            ...
            var user_name = (user.user_name || user.user_email || 'N/A'),
                ...
                user_meta = vendor + user.user_type + last_online + user_info;

            this.data.user_list.append('<li id="FBC_chat_user_' + user.user_id + '" data-id="' + user.user_id + '" data-cnv-id="' + user.conversation_id + '" data-name="' + user_name + '" data-count="0" data-chat="' + user.chat_with + '" class="' + cssclass + '">' +
                '<div class="user-avatar"><img src="' + avatar + '" /></div>' +
                '<i class="fa fa-check-circle"></i>' + is_mobile +
        ...
        notify            : function (title, msg, callback, tag) {
            ...
            if (!( "Notification" in window )) {
                ...
                var notification = new Notification(title, {
                    body: msg,
                    icon: fbc.plugin_url + '/images/fbc-ico.png',
                    tag : tag
                });
        ...
        new_msg           : function (msg, msg_id) {
            ...
            if (self.valid_operator(msg.vendor_id)) {
                ...
                var obj_user = $('#FBC_chat_user_' + msg.user_id),
        ...
        add_msg           : function (msg, last_user_id, last_msg_id) {
            ...
            if (this.objs.cnv) {
                ...
                this.objs.cnv.append('<div id="FBC_msg_' + msg.id + '" class="chat-cnv-line' + cssclass + '">' +
        ...
        manage_reply_box  : function (last_cnv_id) {
            ...
            this.data.ref_cnv.child(this.data.user.conversation_id).on('child_added', function (new_snap) {
                ...
                if (new_snap.val() == 'closed') {
                    ...
                    $('#FBC_cnv_reply').attr('disabled', 'disabled');
        ...
        cnv_msgs_loaded   : function (total_msgs) {
            ...
            if (!total_msgs)
                $('#FBC_load_msg').html(this.strings.msg.no_msg + '.');
            else
                $('#FBC_load_msg').empty();
        ...
        post              : function (action, mode, data, callback) {
            ...
            $.post(fbc.ajax_url + '?action=' + action + '&mode=' + mode, data, callback, 'json')
                .fail(function (jqXHR) {
        ...
        display_ntf       : function (ntf, type) {
            ...
            $('#FBC_popup_ntf').removeClass().addClass('chat-ntf chat-' + type).html(icon + ntf).fadeIn(300);
        ...
        show_welcome_popup: function () {
            ...
            $('#FBC_popup_cnv').addClass('chat-welcome');
            $('#FBC_cnv').hide();
            $('#FBC_cnv_bottom').hide();
            $('#FBC_sidebar_right').hide();
        ...
        valid_operator    : function (vendor_id) {
            ...
            if (!fbc.wcfm_wpv_active) {
                return true
            }

            if (fbc.wcfm_wpv_active && fbc.active_vendor.vendor_id == vendor_id) {
                return true;
            }

            if (fbc.wcfm_wpv_active && fbc.active_vendor.vendor_id == '0' && !fbc.vendor_only_chat) {
                return true;
            }
    ...
    $(document).ready(function () {
        ...
        $(window).resize(function () {
            ...
            var win_h = window.innerHeight,
                ...
                console_w = $('#wpbody-content').width() || $('.wcfm-wcfm-content').width();

            if ($('#wpbody-content').length == 0) {
                console_h = window.innerHeight - (adminbar_h + $('#wcfm_wcfm-header').outerHeight() + 30);

            }

            if (win_w < 766) {

                $('#FBC_console').css('height', '');
                $('#FBC_sidebar_left').css('height', '');
                $('#FBC_popup_cnv').css('height', '');
                $('#FBC_sidebar_right').css('height', '');
                $('#FBC_users').css('height', '');
                $('#FBC_cnv').css('height', 250);
                $('.wcfm-live-chat-console-container').width(console_w - 12);

            } else {

                $('#FBC_console').height(console_h);
                $('#FBC_sidebar_left').height(console_h);
                $('#FBC_popup_cnv').height(console_h);
                $('#FBC_sidebar_right').height(console_h);
                $('#FBC_users').height(console_h - 110);
                $('#FBC_cnv').height(console_h - $('#FBC_cnv_bottom').innerHeight() - 30);
                $('.wcfm-live-chat-console-container').width(console_w - 20);
        ...
        $(window).scroll(function () {
            ...
            if ($('#wpbody-content').length == 0) {
                ...
                if (scroll <= $('#wcfm_wcfm-header').outerHeight()) {
                    console_h = window.innerHeight - (adminbar_h + 30) + scroll;
                    //$('.wcfm-live-chat-console-container').css({'position': 'absolute', 'top': 'auto'});

                } else {
                    console_h = window.innerHeight - (adminbar_h + $('#wcfm_wcfm-header').outerHeight() + 30)

                    //$('.wcfm-live-chat-console-container').css({'position': 'fixed', 'top': console_top});
