...
jQuery(document).ready(function($) {
	...
	$wcfm_ahli_table = $('#wcfm-ahli').DataTable( {
		"processing": true,
		"serverSide": false,
		"responsive": true,
		...
		'ajax': {
			...
			"complete" : function () {
				...
				// Fire delete ahli
				$( document.body ).trigger( 'delete_wcfm-ahli' );
			}
		}
	} );
	...
	// Delete Order
	$( document.body ).on( 'delete_wcfm-ahli', function() {
	
		$('.wcfm_ahli_delete').each(function() {
			
			$(this).click(function(event) {
				event.preventDefault();
				var rconfirm = confirm("Are you sure and want to delete this 'Member'?\nYou can't undo this action ...");
				if(rconfirm) deleteWCFMAhli($(this));
				return false;
			});
		});
	});

	function deleteWCFMAhli(item) {
		
		$('#wcfm-ahli_wrapper').block({
			message: null,
			overlayCSS: {
				background: '#fff',
				opacity: 0.6
		...
		$.ajax({
			...
			success:	function(response) {
				$wcfm_ahli_table.ajax.reload();
				$('#wcfm-ahli_wrapper').unblock();
			}
		});
	}
