...
var JEBookingFormBuilder = new Vue({
	...
	computed: {
		...
		availableFields: function() {
			...
			var fields = [];
			var skipFields = [ 'submit', 'page_break', 'heading', 'group_break', 'repeater_end' ];
...
var JEBookingFormNotifications = new Vue({
	...
	methods: {
		...
		getMailChimpData: function( event, isValidate ) {
			var self = this,
				api_key = this.currentItem.mailchimp.api_key;
		...
		getGetResponseData: function( event, isValidate ) {
			var self = this,
				api_key = this.currentItem.getresponse.api_key;
